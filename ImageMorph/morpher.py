...
image1 = Image.open("ImageMorph/fall_128.png")
image2 = Image.open("ImageMorph/night_128.png")
    ...
    with Bar('Processing...') as bar:  
        ...
        while(True):
            ...
            while indexs[0] == indexs[1]:
                indexs[1] = [np.random.randint(swap_size,size - swap_size),np.random.randint(swap_size,size - swap_size)] # no swapimg same item
            ...
            for x in range(swap_size):
                for y in range(swap_size):
                 ...
                 #print(pos[0][1] + pos[0][0] * size,pos[0][1], " + ",  pos[0][0] * size)
            ...
            if sum(new_compatibilities) > sum(old_compatibilities):
                 for x in range(swap_size):
                    for y in range(swap_size):
                        ...
                        pairs[pos[0][1] + pos[0][0] * size] = swaped_pairs[(x + y * swap_size)//2][0]
                        pairs[pos[1][1] + pos[1][0] * size] = swaped_pairs[(x + y * swap_size)//2][1]
